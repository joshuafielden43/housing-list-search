...
**Default daily posture for housing-list-search:** do **not** install the OCR tier on the cron host. Prefer pdfplumber only. **Runtime opt-in (#1088):** OCR runs only when `HLS_ENABLE_MARKER_PDF=1` (and not disabled). **Daily entrypoint (#1089):** `run_daily.sh` forces `HLS_DISABLE_MARKER_PDF=1` and unsets enable. **Isolation (#1090):** conversion uses a short-lived subprocess so multi-GB torch RSS is not held in the scrape process. Enable OCR only on a capable machine after accepting GPL + weight terms.
