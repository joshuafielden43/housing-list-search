...
| **RunReview** | The canonical operator-attention signals for one Run: Suspicious Zero, overdue Validated Zero, Low-yield, STALE threshold, and SCRAPE_FAILED. It never makes the Run fail (ADR-0004); ordinary target success/failure counts are Run status, not RunReview |
